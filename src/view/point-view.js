...
import { humanizeDateOnly } from '../util.js';


function createNewPointTemplate(point) {
  const {type,name,dateFrom,dateTo,basePrice,title,price, src,isFavorite} = point;
  const dateStart = humanizeTime(dateFrom);
  const dateEnd = humanizeTime(dateTo);
  const date = humanizeDateOnly(dateFrom);
    ...
    <li class="trip-events__item">
      <div class="event">
        ...
        <div class="event__type">
          <img class="event__type-icon" width="42" height="42" src="${src}" alt="Event type icon">
        </div>
